...
CPP_PACKAGE_VERSION=$(grep -Pzo "<version>[^<>]+</version>" "$CPP_PACKAGE_NUSPEC_PATH" | sed -e 's/<.\?[a-zA-Z]\+>//g')
echo "$CPP_PACKAGE_VERSION"
echo "$CPP_PACKAGE_VERSION" > CPP_PACKAGE_VERSION.txt
CPP_PACKAGE_RELEASE_NOTES=$(grep -Pzo "<releaseNotes>[^<>]+</releaseNotes>" "$CPP_PACKAGE_NUSPEC_PATH" | sed -e 's/<.\?[a-zA-Z]\+>//g')
