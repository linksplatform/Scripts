#!/bin/bash
# Test script to verify pagination functionality without actually cloning
...
# Function to fetch all repositories with pagination support (test version)
fetch_all_repositories_test() {
  local org="$1"
  local page=1
  local per_page=100
...
# Test with linksplatform organization
echo "Testing pagination with linksplatform organization..."
repositories_json=$(fetch_all_repositories_test "$1")

# Count and display the first few repository names for verification
echo ""
echo "Repository count verification:"
echo "$repositories_json" | jq length
